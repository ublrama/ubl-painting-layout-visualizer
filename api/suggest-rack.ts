...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (paintingId && targetRack) {
      ...
      for (const rackPainting of rack.paintings) {
        ...
        if (fitsAfterRemoval) {
          ...
          moveSuggestions.push({ painting: rackPainting as PlacedPainting, suggestedRack: suggestedRackName || 'Geen rek gevonden', canFit });
        }
        if (moveSuggestions.length >= 3) break;
      }
      return Response.json({ canPlaceDirectly: false, moveSuggestions } as ForcePlacementResult, { headers: CORS_HEADERS });
    }
    ...
    const mockPainting: Painting = { id: 'mock', signatuur: 'mock', collection: 'Unknown', width, height, depth, assignedRackName: null, manuallyPlaced: false, predefinedRack: null };
    ...
    console.error('[suggest-rack] error:', err);
    return Response.json({ error: 'Internal server error', detail: String(err) }, { status: 500, headers: CORS_HEADERS });
  }
}
